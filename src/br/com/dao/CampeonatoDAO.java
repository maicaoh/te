...
public class CampeonatoDAO {
    ...
    public Campeonato consultarPorNome(String nome) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                camp.setQtdEquipe(rs.getInt("qtd_equipe_max"));
            }
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, e);
        }
    ...
    public boolean atualizarCampeonato(Campeonato campeonato) {
        String sql_consulta_geral = "update campeonato set nome_campeonato = ?,qtd_equipe_max=? where campeonato.id_campeonato = ?";
        Campeonato camp = new Campeonato();
        boolean result = false;
        try {
            pst = Conexao.getConexao().prepareStatement(sql_consulta_geral);
            pst.setString(1, campeonato.getNome());
            pst.setInt(2, campeonato.getQtdEquipe());
            pst.setInt(3, campeonato.getIdCampeonato());
            result = pst.execute();
